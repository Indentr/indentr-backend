...
def insert_treatment_plan(register_and_login):
    ...
    letters_collection.insert_one(treatment_plans[0])
    letters_collection.insert_one(treatment_plans[1])
    letters_collection.insert_one(treatment_plans[2])
    letters_collection.insert_one(treatment_plans[3])
    letters_collection.insert_one(treatment_plans[4])
    return token
...
def test_save_img(insert_treatment_plan):
    ...
    image_data = Image.new("RGB", (100, 100)).tobytes()  # Replace with actual image data
...
def test_save_details(insert_treatment_plan):
    ...
    response = client.post(
        ...
        json={
            ...
            "phone": "999",
            "address": "1 Stamford Bridge London" 
        }
    )
