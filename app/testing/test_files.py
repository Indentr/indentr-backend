...
def register_and_login():
    registerResponse = client.post("/auth/register", json={"name": "John Terry", "email": "johnterry@gmail.com", "password": "password"})
...
def insert_treatment_plan(register_and_login):
    ...
    letters_collection.insert_one(treatment_plan_document_template)
    return token, letter_id, treatment_plan_document_template
...
def test_get_treatment_plan(insert_treatment_plan):
    ...
    # compares the inserted treatment plan against the returned treatment plan
    treatment_plan_json = json.loads(json.dumps(treatment_plan_document_template, default=str))
    # only compares certain columns to make sure they match
    essential_keys = ["_id", "consent_letter", "patient_info", "user_id"]
    for key in essential_keys:
        assert data[key] == treatment_plan_json[key]
...
def test_get_nonexistent_treatment_plan(insert_treatment_plan):
    ...
    data = response.json()
    assert data["detail"] == "Error finding letter"
...
def test_save_treatment_plan(insert_treatment_plan):
    token, letter_id, treatment_plan_document_template = insert_treatment_plan
    treatment_plan_document_template["patient_info"]["dob"] = "1905-02-01"
    response = client.post(
        "/files/saveTreatmentPlan",
        json={
            "treatmentPlan": str(treatment_plan_document_template),
            "letterId": letter_id
        },
        headers={
            "Authorization": f"Bearer {token}",
...
def test_save_nonexistent_treatment_plan(insert_treatment_plan):
    ...
    data = response.json()
    assert data["detail"] == "Letter Id not found in database"
