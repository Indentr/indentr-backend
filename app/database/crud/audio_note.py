...
def retrieve_last_three_notes(user_id: str):
    try:
        notes = AudioNote.objects(user_id=user_id).only("patient_id", "formatted_notes", "transcript", "createdAt").order_by("-_id").limit(3).select_related()
...
def retrieve_patients_last_three_notes(patient_id: str):
    try:
        notes = AudioNote.objects(patient_id=patient_id).only("patient_id", "formatted_notes", "transcript", "createdAt").order_by("-_id").limit(3).select_related()
