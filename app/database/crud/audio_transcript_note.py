...
from app.database.schemas.audio_transcript_note import (
    AudioTranscriptNote,
    TranscriptNotePromptOutputs,
)
from app.database.schemas.patient import Patient, PatientName


def create_audio_transcription_note(
    patient_id: str,
    user_id: str,
    practice_id: str,
    ...
    try:
        ...
        for formatted_note in note_dict["formatted_notes"]:
            notePromptOutput = TranscriptNotePromptOutputs(note_prompt_id=formatted_note["note_prompt_id"], note_text=formatted_note["note_text"])
            formatted_notes.append(notePromptOutput)

        audio_transcription_note = AudioTranscriptNote(
        ...
        audio_transcription_note.save()

        # Return the ID of the created note
        return str(audio_transcription_note.id)  # or audio_note.pk for primary key

    except Exception as e:
        raise HTTPException(status_code=404, detail=str(e)) from None


def delete_transcription_note(practice_id: str, file_id: str):
    note_to_delete = AudioTranscriptNote.objects(id=file_id, practice_id=practice_id).first()
...
def retrieve_all_users_transcription_notes(user_id: str = None, practice_id: str = None):
    try:
        if not user_id:
            # Gets all practices notes
            notes = (
                AudioTranscriptNote.objects(practice_id=practice_id)
                .only("patient_id", "transcript", "createdAt")
                .order_by("-createdAt")
                .select_related()
            )
        else:
            # Gets all users notes
            notes = AudioTranscriptNote.objects(user_id=user_id).only("patient_id", "transcript", "createdAt").order_by("-createdAt").select_related()
...
def retrieve_transcription_note(note_id: str, practice_id: str):
    # Query the note using MongoEngine
    note = (
        AudioTranscriptNote.objects(id=note_id, practice_id=practice_id)
...
def retrieve_last_three_transcription_notes(user_id: str):
    try:
        notes = (
            AudioTranscriptNote.objects(user_id=user_id)
            .only("patient_id", "formatted_notes", "transcript", "createdAt", "input_tokens", "output_tokens", "cost", "model")
            .order_by("-_id")
            .limit(3)
            .select_related(2)
...
def retrieve_transcription_notes_alphabet_status(user_id: str = None, practice_id: str = None):
    try:
        pipeline = [
        ...
        result = AudioTranscriptNote.objects.aggregate(*pipeline)
...
def retrieve_all_users_transcription_notes_filtered_by_char(starts_with: str, user_id: str = None, practice_id: str = None):
    try:
        pipeline = [
            {"$match": {"user_id": ObjectId(user_id)}},
            {"$lookup": {"from": "patients", "localField": "patient_id", "foreignField": "_id", "as": "patient"}},
        ...
        notes = AudioTranscriptNote.objects.aggregate(*pipeline)
...
def retrieve_patients_last_three_transcription_notes(patient_id: str):
    try:
        notes = (
            AudioTranscriptNote.objects(patient_id=patient_id)
...
def retrieve_audio_transcription_note_time_for_billing_cycle(practice_id: str, start_date: datetime, end_date: datetime):
    # audio_notes = AudioTranscriptNote.objects.filter(practice_id=practice_id, createdAt__gte=start_date, createdAt__lte=end_date)
...
def update_transcription_note(note_id: str, noteObj: Dict[str, Any], practice_id: str):
    # Use MongoEngine to find and update the document
    note = AudioTranscriptNote.objects(id=note_id, practice_id=practice_id).first()
...
def update_formatted_transcription_notes(note_id: str, new_formatted_notes: str):
    try:
        # Fetch the complete document
        audio_note = AudioTranscriptNote.objects.get(id=note_id)
